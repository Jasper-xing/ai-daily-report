...
class RSSParser:
    ...
    def parse(self, content: str, feed_url: str = "") -> List[ParsedRSSItem]:
        ...
        Returns:
            解析后的条目列表
        """
        if "api.github.com/search/repositories" in feed_url:
            return self._parse_github_search(content, feed_url)
    ...
    def _parse_github_search(self, content: str, feed_url: str) -> List[ParsedRSSItem]:
        """将 GitHub Repository Search API 响应转换为统一条目。"""
        try:
            data = json.loads(content)
        except json.JSONDecodeError as e:
            raise ValueError(f"GitHub 搜索结果解析失败 ({feed_url}): {e}") from e
        ...
        for repo in data.get("items", []):
            ...
            summary = f"{description} | {language} | {stars} stars" if description else f"{language} | {stars} stars"
            items.append(ParsedRSSItem(
                title=full_name,
                url=url,
                published_at=repo.get("updated_at") or repo.get("pushed_at"),
                summary=summary,
                author=(repo.get("owner") or {}).get("login"),
                guid=str(repo.get("id") or url),
            ))
