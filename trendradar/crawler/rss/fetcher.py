...
class RSSFeedConfig:
    ...
    name: str                   # 显示名称
    url: str = ""               # RSS URL
    type: str = "rss"           # rss | github_search | arxiv
...
class RSSFetcher:
    ...
    def fetch_feed(self, feed: RSSFeedConfig) -> Tuple[List[RSSItem], Optional[str]]:
        ...
        """
        try:
            feed_url = self._build_feed_url(feed)
            headers = {}
            if feed.type == "github_search" and os.environ.get("GITHUB_TOKEN"):
                headers["Authorization"] = f"Bearer {os.environ['GITHUB_TOKEN']}"

            response = self.session.get(feed_url, timeout=self.timeout, headers=headers)
            response.raise_for_status()

            parsed_items = self.parser.parse(response.text, feed_url)
    ...
    @staticmethod
    def _build_feed_url(feed: RSSFeedConfig) -> str:
        """将结构化科技源配置转换为实际 API URL。"""
        if feed.type == "github_search":
            params = {
                "q": feed.q,
        ...
        if feed.type == "arxiv":
            sort_by = {0: "relevance", 1: "lastUpdatedDate", 2: "submittedDate"}.get(
                feed.sort_by, "submittedDate"
            )
            params = {
                "search_query": f"all:{feed.search_query}",
                "start": 0,
                "max_results": feed.count,
                "sortBy": sort_by,
    ...
    def from_config(cls, config: Dict) -> "RSSFetcher":
        ...
        for feed_config in config.get("feeds", []):
            ...
            feed = RSSFeedConfig(
                ...
                type=feed_config.get("type", "rss"),
                max_items=feed_config.get("max_items", 0),  # 0=不限制
                enabled=feed_config.get("enabled", True),
                max_age_days=max_age_days,  # None=使用全局，0=禁用，>0=覆盖
                q=feed_config.get("q", ""),
                per_page=int(feed_config.get("per_page", 10)),
                sort=feed_config.get("sort", "updated"),
                count=int(feed_config.get("count", 5)),
                search_query=feed_config.get("search_query", "AI"),
                sort_by=int(feed_config.get("sort_by", 2)),
            )
            if feed.id and (feed.url or feed.type in {"github_search", "arxiv"}):
                feeds.append(feed)
